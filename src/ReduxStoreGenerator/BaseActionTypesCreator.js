...
export default function BaseActionTypesCreator(
  storeName: string,
  ActionTypesMap: {[string]: string},
): {[string]: string} {
  console.log(storeName, ActionTypesMap)
  const mappedType = {
  ...
  const descriptor = Object.keys(mappedType).reduce(
    (prev, typeName) => {
      prev[typeName] = {
        writable: false,
        enumerable: true,
